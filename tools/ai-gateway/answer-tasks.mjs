...
export function buildPrompt(promptFile, review = {}) {
  ...
  if (review.mode === "visual_audit") {
    ...
    "指针式仪表必须先逐个追踪实际连接导线的接线柱，据此选择量程；再确认对应数字刻度、分度值、指针格数和读数。小格按相邻刻线之间的间隔计数，零刻线只是边界，不得把零刻线计作第一个小格；双排刻度必须分别读数并核对固定倍率关系。禁止先看指针再猜量程。电压表/电流表的确定数值必须在内部完成结构化证据链（图号、实际端子、量程、刻度圈、分度值、相邻刻线、左右双向读数、最后的计算校验）；指针存在最小分度争议时，以几何中心线是否落在刻线为准，中心线不能区分或双向读数不一致时输出【视觉证据不足，需复核】，不得硬猜或用 $R=U/I$ 反推读数。\n" +
      "刻度尺和弹簧必须分别读取物体两端刻度后相减；钩码必须逐个计数，并用单个钩码重力交叉校验总拉力。停表必须先确认分钟小盘和秒盘各自的零点、刻度方向、指针中心线与单位，再写出‘分钟盘 × 60 s + 秒盘’的合读链；有两组印刷数字时不得直接取相邻数字，不能只读秒盘。\n" +
  ...
  const generationTask = `${specification}\n\n---\n\n# 当前真实试卷生成任务\n\n` +
    ...
    "先绑定小问明确引用的目标图，再追踪该图内的接线或方向关系，随后确认量程、分度值和指针位置，禁止仅凭题型惯例作答。电压表/电流表的确定数值必须在内部完成结构化证据链（目标图号、该图是否显示实际导线、该图自身可见的端子或“不适用/未显示”、量程、刻度圈、分度值、相邻刻线、左右双向读数、最后的计算校验）。指针存在最小分度争议时，以几何中心线是否落在刻线为准，中心线不能区分或双向读数不一致时输出【视觉证据不足，需复核】，不得硬猜或用 $R=U/I$ 反推读数。独立表盘按本图印刷刻度读数，禁止借用同题另一图的接线或量程；若任一步无法从目标图确认，输出【视觉证据不足，需复核】，不得用 $R=U/I$ 反推读数。停表必须先确认分钟小盘和秒盘各自的零点、刻度方向、指针中心线与单位，再写出‘分钟盘 × 60 s + 秒盘’的合读链；有两组印刷数字时不得直接取相邻数字，不能只读秒盘。\n" +
...
export function normalizeAnswerMarkdown(value) {
  ...
  // Models sometimes leave one inline math fence open across several Markdown
  // lines. That makes the renderer lay out the whole derivation as one long
  // inline formula and can strand the final punctuation on its own line.
  // Close and reopen each non-empty formula line, using a Markdown hard break.
  const normalizedMultilineMath = answerOnly.replace(/\$([\s\S]*?)\$/gu, (match, body) => {
    const segments = body.split("\n");
    if (!body.includes("=") || segments.some((segment) => segment.trim().length === 0)) {
      return match;
    }
    return segments.map((segment) => "$" + segment.trim() + "$").join("  \n");
  });
  return normalizedMultilineMath
    .replace(/\$([A-Z](?:['′])?(?:[、，][A-Z](?:['′])?)+)\$/g, "$1")
