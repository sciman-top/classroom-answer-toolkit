...
- 已落地离线 `DeliveryQuestionCoverage -> DeliveryDecisionAggregate` 编译器，以及受控 aggregate manifest 附着和 preimage/result receipt；当前仅完成本地 repo-side attach，尚未接入 WPF 正向 trust 投影。
...
- An offline `DeliveryQuestionCoverage -> DeliveryDecisionAggregate` compiler verifies byte hashes, snapshot binding, sample-package question inventory, and per-question decisions. A controlled CLI can attach that aggregate with a preimage/result receipt, and .NET orchestration can explicitly reverify the source-aware chain; WPF and automatic readers remain fail-closed until verified positive projection is integrated.
...
WPF 当前完成最新交付的 review/trust 投影，以及本地题目级 `DecisionRecord` 的受控附着、刷新和打开；附着工具不生成审批、不推进 lifecycle。离线 aggregate 可通过 preimage/result receipt 受控附着到 manifest，但尚未接入 WPF，因此当前桌面入口不能把交付提升为 `trusted=true`。这不等于视觉网关已接入默认答题流程，也不等于完整 workflow 或 live acceptance 已完成。
