...
- `tools/visual-component-semantics/`: 只从独立 synthetic declaration、current structure result 和 2x crop bytes 编译有限 pointer/major-tick component semantics，不执行自动检测、刻度解释或题目绑定。
- `tools/visual-scale-lattice/`: 从独立 synthetic lattice declaration 与 current component/structure bytes 推导 relative subdivision index，不生成物理量、单位、题目答案或 trust。
...
原题生成侧现有一条显式执行的 provider-backed runtime：它把 hash-bound public problem 与当前 subject-pack spec 发送给既有 text gateway，并将候选原子写入仓外新目录；request、CLI 与 gateway 三层均须允许 cloud egress。当前仅由 local mock 验证 Responses/Chat Completions 与 failover，未接 WPF 默认流程、未做 live gateway 验证，候选固定 `pending_review / trusted=false`。
...
Word 输入当前新增一条严格受限的 repo-side synthetic 路径：`image_backed_single_page_only` 适配器只解析一份 canonical DOCX 的 OPC/OOXML 关系并提取唯一内部 PNG 为一页 `NormalizedPage`。它拒绝正文、表格、OMML、显式分页、多图、外部图片关系和非规范包结构；不重建普通 Word 排版，不接默认工作流、gateway、trust、workstation 或 live acceptance。
