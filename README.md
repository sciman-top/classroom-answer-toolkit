...
- `tools/sample-flywheel/`: 合成样例的 index/package 准入、plumbing/scoring 门禁与 `SampleRunRecord` 编译；当前 scoring 只做 SHA-256 exact-diff 和 fixture 标注记账。
...
样例飞轮现已具备完全合成 fixture 的首个可执行准入与记账闭环：`样例交付/index.json` 是不可覆盖的 canonical authority，并通过 `subjectPack / packageRef / packageSha256` 绑定唯一 structured package，通过 `candidateBindings` 绑定 negative-candidate descriptor bytes；hash-bound 样例资产由 `.gitattributes` 固定 LF。package 与内部引用必须留在对应 canonical root。`plumbing` 不产优化信号，`scoring` 要求显式 candidate/truth/leakage 状态并受 fail-closed 门禁约束。输出通过仓内有限 shape validator、compiler semantic invariants 和当前 canonical authority bytes 重验，不代表完整 Draft 2020-12 或任意归档 authority 校验；当前比较器仅证明 byte/hash 差异和预标注根因记账，不等于语义答案评分、优化候选生成或灰度放行。
