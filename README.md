...
- 已落地离线 `DeliveryQuestionCoverage -> DeliveryDecisionAggregate` 编译器，用原始 bytes SHA-256、snapshot、sample-package 题目 inventory 和逐题 DecisionRecord 证明全题覆盖；当前仅是合成合同能力，尚未接入 manifest/WPF 正向 trust 投影。
...
- An offline `DeliveryQuestionCoverage -> DeliveryDecisionAggregate` compiler verifies byte hashes, snapshot binding, sample-package question inventory, and per-question decisions; this is synthetic contract capability only and is not yet connected to positive manifest/WPF trust projection.
...
WPF 当前完成最新交付的 review/trust 投影，以及本地题目级 `DecisionRecord` 的受控附着、刷新和打开；附着工具只更新 manifest 的决策引用与 fail-closed 状态投影，不生成审批、不推进 lifecycle。离线 aggregate 尚未接入该入口，因此不能把交付提升为 `trusted=true`。这不等于视觉网关已接入默认答题流程，也不等于完整 workflow 或 live acceptance 已完成。
离线 delivery aggregate 已能对合成 `sample-package` inventory、snapshot/input/manifest bytes 和逐题决策做完整覆盖证明，但它尚未接入现有 attach/WPF；因此正向 aggregate fixture 不能被表述为真实交付已可信或 workflow/live acceptance 已完成。
