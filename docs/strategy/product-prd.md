...
- GEN-005 将 GEN-004 接入 WPF 显式工作流：用户选择 request/workspace/config/仓外新目录并单独授权云外发，编排器在启动前与生成后双重 fail closed；成功只回填 `answer.md` 和 subject-pack 到既有交付输入，交付仍是独立命令。该切片构成 repo-side workflow integration，不构成真实 gateway verification、答案正确、review approval、workstation accepted 或 live accepted。
...
- VISION-025 只用独立 public synthetic declaration 和 VISION-024/008 geometry 建立相对 scale lattice：5 subdivisions/major interval、120 px major spacing、24 px subdivision spacing，并推导 pointer 位于 origin 后第 11 个 subdivision；`11` 只是无量纲相对索引，不是物理读数、单位或答案，不构成自动刻度理解、Track、trust 或 live authority。
- VISION-026 只对一份 public synthetic、单页、单内部 PNG 的 canonical DOCX 执行 OPC/OOXML 关系解析和原图提取，输出一页 hash-bound `NormalizedPage`；它不是普通段落/表格/OMML/分页布局重建，不构成通用原生 Word 支持、OCR/layout/Track、默认 workflow、gateway/workstation verification 或 live acceptance。
